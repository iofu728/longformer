...
class Longformer(RobertaModel):
    def __init__(self, config):
        ...
        self.spa = LongformerSparseAttention(True)
    ...
    def forward(self, attention_mask=None, dynamic=None, **kwargs):
...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        q = self.query(hidden_states.reshape(bsz, seq_len, -1)).reshape(seq_len, bsz, -1)
        k = self.key(hidden_states.reshape(bsz, seq_len, -1)).reshape(seq_len, bsz, -1)
        v = self.value(hidden_states.reshape(bsz, seq_len, -1)).reshape(seq_len, bsz, -1)
        ...
        # assert False
        context_layer = self.predict(q, k, v)
